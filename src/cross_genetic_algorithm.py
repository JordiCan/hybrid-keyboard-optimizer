...
# Keyboard layouts globales - FIX: Make qwerty a flat list
qwerty_2d = [
    ['q', 'w', 'e', 'r', 't', 'y', 'u', 'i', 'o', 'p'],
    ['a', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l', ';'],
    ['z', 'x', 'c', 'v', 'b', 'n', 'm', ',', '.', "'"]
]
# Flatten qwerty to be consistent with other layouts
qwerty = [char for row in qwerty_2d for char in row]
...
def save_results(output_lines, filename):
    """Save results to 'results/ag/' directory"""
    results_dir = os.path.join(os.getcwd(), 'results', 'ag')
...
def run_experiments():
    ...
    TEXT_FILE = 'data/processed/moby_dick_cln.txt'
    SEED = 123
